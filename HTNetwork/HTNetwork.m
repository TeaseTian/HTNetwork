...
+ (NSString *)HTnetworking_md5:(NSString *)string;
...
static HTResponseType sg_responseType = kHTResponseTypeJSON;
static HTRequestType  sg_requestType  = kHTRequestTypeJSON;
static HTNetworkStatus sg_networkStatus = kHTNetworkStatusUnknown;
...
+ (HTURLSessionTask *)_requestWithUrl:(NSString *)url
                         refreshCache:(BOOL)refreshCache
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            HTAppLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
    ...
    if (httpMethod == 1) {
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == kHTNetworkStatusNotReachable ||  sg_networkStatus == kHTNetworkStatusUnknown ) {
                    id response = [HTNetwork cahceResponseWithURL:absolute
                                                          parameters:params];
                    if (response) {
                        if (success) {
...
+ (HTURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        HTAppLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        HTAppLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (HTURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                HTAppLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                HTAppLog(@"Download fail for url %@, reason : %@",
                         [self absoluteUrlWithPath:url],
                         [error description]);
            }
        }
...
+ (void)detectNetwork{
    ...
    [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        if (status == AFNetworkReachabilityStatusNotReachable){
            sg_networkStatus = kHTNetworkStatusNotReachable;
        }else if (status == AFNetworkReachabilityStatusUnknown){
            sg_networkStatus = kHTNetworkStatusUnknown;
        }else if (status == AFNetworkReachabilityStatusReachableViaWWAN){
            sg_networkStatus = kHTNetworkStatusReachableViaWWAN;
        }else if (status == AFNetworkReachabilityStatusReachableViaWiFi){
            sg_networkStatus = kHTNetworkStatusReachableViaWiFi;
        }
    }];
}
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        NSString *key = [NSString HTnetworking_md5:absoluteURL];
        NSString *path = [directoryPath stringByAppendingPathComponent:key];
        
        NSData *data = [[NSFileManager defaultManager] contentsAtPath:path];
        if (data) {
            cacheData = data;
            HTAppLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:directoryPath
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:&error];
            if (error) {
                HTAppLog(@"create cache dir error: %@\n", error);
                return;
            }
        }
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                HTAppLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                HTAppLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
